...
class BrowserUseTool(BaseTool, Generic[Context]):
    def _replace_secure_credentials(self, text: str) -> str:
        """Remplace les références sécurisées par leurs valeurs.
        
        Args:
            text: Le texte contenant potentiellement des références sécurisées (#service_key#)
            
        Returns:
            Le texte avec les références remplacées par leurs valeurs
        """
        ...
        def replace_match(match):
            ...
            if value is None:
                logging.warning(f"Identifiant non trouvé: {service}.{key}")
                return match.group(0)  # Retourne le texte original si non trouvé
            return value
            
        # Recherche les motifs #service_key#
        ...
        async with self.lock:
            try:
                ...
                if action == "go_to_url":
                    ...
                    # Remplacer les identifiants sécurisés dans l'URL si nécessaire
                    processed_url = self._replace_secure_credentials(url)
                    if processed_url != url:
                        logger.info("URL traitée avec des identifiants sécurisés")
                
                    page = await context.get_current_page()
                    await page.goto(processed_url)
                ...
                # Element interaction actions
                    ...
                    # Remplacer les identifiants sécurisés dans le texte
                    processed_text = self._replace_secure_credentials(text)
                    if processed_text != text:
                        logger.info("Identifiants sécurisés détectés et remplacés")
                
                    await context._input_text_element_node(element, processed_text)
                    return ToolResult(
                        output=f"Input entered into element at index {index} (contenu masqué pour des raisons de sécurité)"
                    )   
